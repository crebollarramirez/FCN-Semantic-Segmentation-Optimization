...
def train():
    ...
    for epoch in range(epochs):
        ...
        # Early stopping
        if epochs_no_improve >= patience and early_stop_epoch is None:
            early_stop_epoch = int(epoch)
            print(f"early_stop_epoch value: {early_stop_epoch}")
...
def plot_losses(train_losses, val_losses, best_epoch, early_stop_epoch):
    ...
    plt.axvline(best_epoch+1, color='r', linestyle='--', label='Best Model')
    if early_stop_epoch is not None:
        print(f"Early stopping epoch value for plotting: {early_stop_epoch}")
        plt.scatter(early_stop_epoch+1, val_losses[early_stop_epoch], color='blue', marker='x', s=100, label='Early Stop')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('Loss vs. Epochs')
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_vs_epochs.png')  # Save the plot as a PNG file
